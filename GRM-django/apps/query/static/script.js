document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadQueryState(state) {
        ...
        if (state.where && state.where.rules && state.where.rules.length > 0) {
            const rootGroupEl = whereContainer.querySelector('.filter-group');
            rootGroupEl.querySelector('.rules-container').innerHTML = ''; 
            rebuildWhereClauseUI(state.where, rootGroupEl); 
        }
    ...
    function rebuildWhereClauseUI(groupData, groupEl) {
        const condition = (groupData.condition || 'AND').toUpperCase();
        groupEl.querySelector(`input[value="${condition}"]`).checked = true;

        const rulesContainer = groupEl.querySelector('.rules-container');
        
        (groupData.rules || []).forEach(rule => {
            if (rule.condition) {
                groupEl.querySelector('.add-group-btn').click();
                const newGroupEl = rulesContainer.lastElementChild;
                newGroupEl.querySelector('.rules-container').innerHTML = '';
                rebuildWhereClauseUI(rule, newGroupEl);
            } else {
                ...
                if (op === 'between' || op === 'not between') {
                    ...
                    if (typeof rule.value === 'object' && rule.value !== null && rule.value.model) {
                        ...
                        valueInput.classList.add('subquery-active');
                    } else {
